  ...
  void print() {
    ...
    if (m_signature > CCC_MAX_SIGNATURE) // color code! (CC)
    {
      ...
      printf("CC block sig: %s (%d decimal) x: %d y: %d width: %d height: %d "
             ...
             sig, m_signature, m_x, m_y, m_width, m_height, m_angle, m_index,
             m_age);
...
public:
  ...
  int8_t getBlocks(bool wait = true, uint8_t sigmap = CCC_SIG_ALL,
...
int8_t Pixy2CCC<LinkType>::getBlocks(bool wait, uint8_t sigmap,
                                     uint8_t maxBlocks) {
  blocks = NULL;
  numBlocks = 0;
