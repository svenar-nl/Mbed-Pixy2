...
// version 2. (TPixy2).  TPixy takes a communication link as a template
// parameter so that all communication modes (SPI, I2C and UART) can share the
// same code.
//
...
struct Intersection {
  void print() {
    ...
    for (i = 0; i < m_n; i++) {
      printf("  %d: index: %d angle: %d\n", i, m_intLines[i].m_index,
             m_intLines[i].m_angle);
    }
  }
...
struct Barcode {
  void print() {
    printf("Barcode: (%d %d), val: %d flags: %d\n", m_x, m_y, m_code, m_flags);
  }
...
public:
  ...
  int8_t getMainFeatures(uint8_t features = LINE_ALL_FEATURES,
                         bool wait = true) {
...
template <class LinkType> int8_t Pixy2Line<LinkType>::setMode(uint8_t mode) {
  ...
  *(int8_t *)m_pixy->m_bufPayload = mode;
  ...
  m_pixy->sendPacket();
  if (m_pixy->recvPacket() == 0 &&
      m_pixy->m_type == PIXY_TYPE_RESPONSE_RESULT && m_pixy->m_length == 4) {
